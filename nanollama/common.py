...
# Override with NANOLLAMA_DTYPE env var: "bfloat16", "float16", "float32"
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
def _detect_compute_dtype():
    env = os.environ.get("NANOLLAMA_DTYPE")
    if env is not None:
        return _DTYPE_MAP[env], f"set via NANOLLAMA_DTYPE={env}"
    if torch.cuda.is_available():
        ...
        # Users can still force fp16 via NANOLLAMA_DTYPE=float16 if they know what they're doing.
