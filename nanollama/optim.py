...
Some of the changes in nanollama implementation:
...
# -----------------------------------------------------------------------------
# Pure AdamW optimizer for architectures that don't use Muon.
# Reuses the adamw_step_fused kernel for efficiency.

class PureAdamW(torch.optim.Optimizer):
    """
    Pure AdamW optimizer. Only supports kind='adamw' param groups.

    Lightweight alternative for architectures that don't need Muon
    (no momentum stacking, no Polar Express, no Muon 0-D tensors).
    """
    ...
    def step(self):
        for group in self.param_groups:
            assert group['kind'] == 'adamw', f"PureAdamW only supports kind='adamw', got '{group['kind']}'"
