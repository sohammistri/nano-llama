...
from nanollama.models.base import Linear
from nanollama.dataloader import tokenizing_distributed_data_loader_bos_bestfit, tokenizing_distributed_data_loader_with_state_bos_bestfit
from nanollama.common import compute_init, compute_cleanup, print0, DummyWandb, print_banner, get_base_dir, autodetect_device_type, get_peak_flops, COMPUTE_DTYPE, COMPUTE_DTYPE_REASON, is_ddp_initialized
from nanollama.tokenizer import get_tokenizer, get_token_bytes
...
from nanollama.flash_attention import HAS_FA3
...
# Model architecture
parser.add_argument("--arch", type=str, default="gpt", help="model architecture registered in nanollama.models")
parser.add_argument("--depth", type=int, default=20, help="depth of the Transformer model")
parser.add_argument("--aspect-ratio", type=int, default=64, help="model_dim = depth * aspect_ratio")
parser.add_argument("--head-dim", type=int, default=128, help="target head dimension for attention")
parser.add_argument("--max-seq-len", type=int, default=2048, help="max context length")
parser.add_argument("--window-pattern", type=str, default="SSSL", help="sliding window pattern tiled across layers: L=full, S=half context (e.g. 'SSL')")
parser.add_argument("--embd-pdrop", type=float, default=0.1, help="GPT-2 original embedding dropout")
parser.add_argument("--resid-pdrop", type=float, default=0.1, help="GPT-2 original residual dropout")
parser.add_argument("--attn-pdrop", type=float, default=0.1, help="GPT-2 original attention dropout")
parser.add_argument("--tie-word-embeddings", action=argparse.BooleanOptionalAction, default=True, help="GPT-2 original weight tying between token embedding and lm_head")
parser.add_argument("--bias", action=argparse.BooleanOptionalAction, default=True, help="GPT-2 original linear and LayerNorm bias")
...
if using_fa3:
    ...
    if args.arch == "gpt" and args.window_pattern != "L":
...
def build_model_meta(depth):
    ...
    if args.arch == "gpt":
        config_kwargs["window_pattern"] = args.window_pattern
    elif args.arch == "gpt2_original":
    ...
    config, model_cls = build_model_from_config(args.arch, config_kwargs)
...
default_output_dirname = f"d{args.depth}" if args.arch == "gpt" else f"{args.arch}-d{args.depth}"
output_dirname = args.model_tag if args.model_tag else default_output_dirname # e.g. d12 or gpt2_original-d12
...
if args.fp8:
    if device_type != "cuda":
        ...
        from nanollama.fp8 import Float8LinearConfig, convert_to_float8_training
...
if batch_ratio != 1.0:
    # SGD: linear scaling with batch size is standard (not used in nanollama)
